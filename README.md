...
* [`src/visualize_ml/`](src/visualize_ml/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/visualize_ml/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `visualize-ml` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/visualize_ml/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns visualize-ml.example
  (:require [visualize-ml.config :as config])
